...
func main() {
	...
	port := flag.Int("port", 8001, "Port for SSE transport")
	flag.Parse()

	log := sharedlog.Init("filesystem")
	...
	log.Info().
		Str("root", fs.Root()).
		Int64("max_size", fs.MaxSize()).
		Bool("allow_delete", fs.AllowDelete()).
		Str("transport", *transport).
		Msg("filesystem MCP server starting")
	...
	case "sse":
		sse := server.NewSSEServer(fs.MCP(),
